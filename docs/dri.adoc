...
multiple files (`xxxA.DAT`, `xxxB.DAT`, ...), multiple DAT files can be
specified.

The following information are displayed for each file in the archive:

* index
* volume id (1 for `xxxA.DAT`, 2 for `xxxB.DAT`, etc.)
* timestamp
* size
* filename
...
In this form, _drifile_ must end with "a.dri". This form creates a new DAT
...
are stored in `...a.dri`, files with volume number 2 are stored in `...b.dri`,
and so on.
...
=== dri extract
Usage: *dri extract* [_options_] _drifile_... [--] [(_index_|_filename_)...]
...
=== dri dump
Usage: *dri dump* _drifile_... [--] (_index_|_filename_)
...
*dri compare* compares contents of two DAT archives, ignoring timestamp
