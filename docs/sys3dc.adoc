...
== Synopsis
*sys3dc* [_options_] _drifile_... [_AG00.DAT_]

== Description
`sys3dc` is a decompiler for AliceSoft's System 1-3 game engine. It reads
scenario files (`ADISK.DAT`, `BDISK.DAT`, ...) and optional `AG00.DAT` file.
It then generates source files that can be compiled with
xref:sys3c.adoc[*sys3c(1)*].
